...
if(!is.element("snakemake",ls())){
  projFolder <- "/scratch/project_2005619/bovreg_full/"
...
pipeSMK <- readLines(file.path(pipelineFolder,"Snakebite-miRNA.smk"))
mirdbstat <- readLines(file.path(projFolder, "STATS", "mirdb.stats"))
...
if(sampleinfo.file!=""){
    if(file.exists(sampleinfo.file)){
        sampleInfo <- read.table(sampleinfo.file, sep="\t", header=TRUE)
        sampleInfo.avail <- TRUE  
        colnames(sampleInfo)[1] <- "sampleID"
        if(is.element("DEgroup", colnames(sampleInfo))) do.DEtesting <- TRUE 
  }
}
```
...
font.size <- 1
if(length(samples)>25) font.size <- 0.5
...
out <- data.frame(c("mirDB miRNA:",
                    "Species-spec. miRNA:",
                    "mirDB hairpin:",
                    "Species-spec. hairpin:"
                    ),
                  c(prettyNum(mirdbstat[1], big.mark = ","),
                    prettyNum(mirdbstat[2], big.mark = ","),
                    prettyNum(mirdbstat[3], big.mark = ","),
                    prettyNum(mirdbstat[4], big.mark = ",")
                    ))
...
par(oma=c(6,5,0,0))
labels <- gsub("_R1.fastq.gz.report","",conc.reports.files)
plotThis <- sapply(conc.reports,nrow)
barplot(plotThis, ylab="Concatenated files", las=2, names=labels, col=reportColours[3], yaxt="n", cex.names=font.size)
axis(2, at=seq(0,max(plotThis)), las=2)
```
...
names(raw.reads) <- gsub(".wcl", "", names(raw.reads))
raw.reads <- raw.reads / 4
```

```{r}
par(mar=c(15,5,2,0))
barplot(raw.reads, las=2, main="Raw, lane-wise reads", col=reportColours[1], cex.names=font.size)
```
...
names(trimmed.reads) <- gsub("_trimmed.wcl", "", names(trimmed.reads))
trimmed.reads <- trimmed.reads / 4
```

```{r}
par(mar=c(15,5,2,0))
barplot(trimmed.reads, las=2, main="Trimmed, lane-wise reads", col=reportColours[2], cex.names=font.size)
```
...
barplot(rbind(raw.reads,trimmed.reads), beside=TRUE, las=2, main="Raw and Trimmed, lane-wise reads", col=c(reportColours[1], reportColours[2]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Raw","Trimmed"), fill=c(reportColours[1],reportColours[2]))
```
...
names(conc.reads) <- gsub("_R1.wcl", "", names(conc.reads))
conc.reads <- conc.reads / 4
```

```{r}
par(mar=c(15,6,2,0))
barplot(conc.reads, las=2, main="Concatenated reads", col=reportColours[3], cex.names=font.size)
```
...
names(raw.bases) <- gsub(".wcc", "", names(raw.bases))
```

```{r}
par(mar=c(15,6,2,0))
barplot(raw.bases, las=2, main="Raw, lane-wise bases", col=reportColours[3], cex.names=font.size)
```
```{r}
par(mar=c(15,6,2,0))
barplot(raw.bases/raw.reads, las=2, main="Raw, lane-wise bases per read", col=reportColours[3], cex.names=font.size)
```
...
names(trimmed.bases) <- gsub("_trimmed.wcc", "", names(trimmed.bases))
```

```{r}
par(mar=c(15,6,2,0))
barplot(trimmed.bases, las=2, main="Trimmed, lane-wise bases", col=reportColours[3], cex.names=font.size)
```

```{r}
par(mar=c(15,6,2,0))
barplot(raw.bases-trimmed.bases, las=2, main="Number of trimmed bases per lane", col=reportColours[3], cex.names=font.size)
```

```{r}
par(mar=c(15,6,2,0))
barplot(trimmed.bases/trimmed.reads, las=2, main="Trimmed, lane-wise bases per read", col=reportColours[3], cex.names=font.size)
```
...
```{r}
par(mar=c(15,5,2,8), xpd=TRUE)
barplot(rbind(raw.bases,trimmed.bases), beside=TRUE, las=2, main="Raw and Trimmed, lane-wise bases", col=c(reportColours[1], reportColours[2]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Raw","Trimmed"), fill=c(reportColours[1], reportColours[2]))
```
...
barplot(plotThis, beside=TRUE, las=2, main="Avg read length in bases", col=c(reportColours[1], reportColours[2]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Raw","Trimmed"), fill=c(reportColours[1], reportColours[2]))
```
...
names(conc.bases) <- gsub("_R1.wcc", "", names(conc.bases))
```

```{r}
par(mar=c(15,6,2,0))
barplot(conc.bases, las=2, main="Concatenated Bases", col=reportColours[3], cex.names=font.size)
```
...
barplot(rbind(conc.reads,trna.unmapped.reads, trna.mapped.reads), beside=TRUE, las=2, main="Concatenated and tRNA reads", col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Conc.","tRNA (unmapped)", "tRNA (mapped)"), fill=c(reportColours[1],reportColours[2],reportColours[3]))
```
...
```{r}
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(rbind(conc.bases,trna.unmapped.bases, trna.mapped.bases), beside=TRUE, las=2, main="Concatenated and tRNA bases", col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Conc.","tRNA (unmapped)", "tRNA (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
```{r}
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(rbind(trna.unmapped.reads,phix.unmapped.reads, phix.mapped.reads), beside=TRUE, las=2, main="tRNA filtered and PhiX reads", col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Conc.","PhiX (unmapped)", "PhiX (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
```{r}
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(rbind(trna.unmapped.bases,phix.unmapped.bases, phix.mapped.bases), beside=TRUE, las=2, main="tRNA filtered and PhiX bases", col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("tRNA filtered","PhiX (unmapped)", "PhiX (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
barplot(rbind(phix.unmapped.reads,mature.unmapped.reads, mature.mapped.reads), beside=TRUE, las=2, main="Filtered reads 
        aligned to Mature.fa", col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Total reads","Mature.fa (unmapped)", "Mature.fa (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
```{r}
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(rbind(phix.unmapped.bases, mature.unmapped.bases, mature.mapped.bases), beside=TRUE, las=2, main="Filtered bases aligned to Mature.fa", col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Total reads","Mature.fa (unmapped)", "Mature.fa (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
barplot(rbind(phix.unmapped.reads,mature_species.unmapped.reads, mature_species.mapped.reads), beside=TRUE, las=2, main="Filtered reads 
        aligned to Mature_Species.fa", col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Total reads","Mature_Species.fa (unmapped)", "Mature_Species.fa (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
```{r}
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(rbind(phix.unmapped.bases, mature_species.unmapped.bases, mature_species.mapped.bases), beside=TRUE, las=2, main="Filtered bases aligned to Mature_Species.fa", col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Total reads","Mature_Species.fa (unmapped)", "Mature_Species.fa (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
```{r}
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(mature.mapped.reads - mature_species.mapped.reads, las=2, main="Reads that align more to Mature.fa vs Mature_Species.fa", col=c(reportColours[3]), cex.names=font.size)
```
...
```{r}
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(rbind(mature.unmapped.reads, hairpin.unmapped.reads, hairpin.mapped.reads), beside=TRUE, las=2, main="Mature.fa unaligned reads aligned to Hairpin.fa", col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Mature.fa (unmapped)","Hairpin.fa (unmapped)", "Hairpin.fa (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
```{r}
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(rbind(mature.unmapped.bases, hairpin.unmapped.bases, hairpin.mapped.bases), beside=TRUE, las=2, main="Mature.fa unaligned bases aligned to Hairpin.fa", col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Mature.fa (unmapped)","Hairpin.fa (unmapped)", "Hairpin.fa (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(rbind(hairpin.unmapped.reads, reference.bowtie.unmapped.reads, reference.bowtie.mapped.reads), beside=TRUE, las=2, main=useTitle, col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Hairpin.fa (unmapped)","Reference (unmapped)", "Reference (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(rbind(hairpin.unmapped.bases, reference.bowtie.unmapped.bases, reference.bowtie.mapped.bases), beside=TRUE, las=2, main=useTitle, col=c(reportColours[1], reportColours[2], reportColours[3]), cex.names=font.size)
legend("topright", inset=c(-0.2,0), legend=c("Hairpin.fa (unmapped)","Reference (unmapped)", "Reference (mapped)"), fill=c(reportColours[1], reportColours[2], reportColours[3]))
```
...
barplot(starlogs.matrix, col=2:7, main = "STAR alignment stats (Mature.fa)", las=2, cex.names=font.size)
...
barplot(starlogs.matrix, col=2:7, main = "STAR alignment stats (Mature_species.fa)", las=2, cex.names=font.size)
legend("topright", inset=c(-0.5,0), legend=gsub("starlogs.","",rownames(starlogs.matrix)), fill=c(2:7))
```
...
barplot(starlogs.matrix, col=2:7, main = "STAR alignment stats (Reference)", las=2, cex.names=font.size)
legend("topright", inset=c(-0.5,0), legend=gsub("starlogs.","",rownames(starlogs.matrix)), fill=c(2:7))
```
...
```{r}
barplot(starlogs.totalReads - apply(starlogs.matrix,2,sum), las=2, main="Unconsidered reads in above barplot", col=reportColours[1], cex.names=font.size)
```
...
## tRNA
Despite the filtering against the tRNA database, we also quantify the different tRNAs and base further downstream analyses on the quantifications.

```{r}
trna.files <- list.files(file.path(projFolder,"STATS", "BOWTIE", "tRNA"), pattern="*idxstats")
trna.samples <- gsub("_tRNA.idxstats", "", trna.files)
trna.quant <- read.table(file.path(projFolder,"STATS", "BOWTIE", "tRNA", trna.files[1]))[,1:3]
colnames(trna.quant) <- c("tRNA", "Length", trna.samples[1])

for(i in 2:length(trna.files)){
tmp <- read.table(file.path(projFolder,"STATS", "BOWTIE", "tRNA", trna.files[i]))[,c(1,3)]
colnames(tmp) <- c("tRNA", trna.samples[i])

trna.quant <- merge(trna.quant, tmp, by="tRNA")
}

trna.quant <- trna.quant[- which(trna.quant$tRNA=="*"),]

trna.quant.rpkm <- t(t(trna.quant[,-c(1:2)])/(apply(trna.quant[,-c(1:2)],2,sum)/10^6) )

trna.quant.rpkm <- trna.quant.rpkm/(trna.quant$Length/10^3)
rownames(trna.quant.rpkm) <- trna.quant$tRNA
```

```{r}
datatable(trna.quant.rpkm)
```

```{r export trna counts}
write.table(trna.quant, file.path(projFolder, "QUANTIFICATION","trna_counts.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
write.table(data.frame(ID=rownames(trna.quant.rpkm),trna.quant.rpkm), file.path(projFolder, "QUANTIFICATION","trna_rpkm.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```
...
```{r}
barplot(apply(star.mature[,-1],2,sum), las=2, main="Reads aligned to Mature.fa", col=reportColours[1], cex.names=font.size)
```
...
for(i in 2:length(mirbase_mature_quant.files)){
  ...
  mirbase_mature_quant <- merge(mirbase_mature_quant, tmp, by="miRNA", all.x=TRUE)
}

mirbase_mature_quant[is.na(mirbase_mature_quant)] <- 0
...
```{r}
write.table(mirbase_mature_quant, file.path(projFolder, "QUANTIFICATION","mirbase_mature_star-counts.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)

write.table(data.frame(ID=rownames(mirbase_mature_quant.rpkm),mirbase_mature_quant.rpkm), file.path(projFolder, "QUANTIFICATION","mirbase_mature_star-rpkm.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```
...
```{r}
barplot(apply(star.mature_species[,-1], 2, sum), las=2, main="Reads aligned to Mature_Species.fa", col=reportColours[1])
```
...
for(i in 2:length(mirbase_mature_species_quant.files)){
  ...
  mirbase_mature_species_quant <- merge(mirbase_mature_species_quant, tmp, by="miRNA", all=TRUE)
}
mirbase_mature_species_quant[is.na(mirbase_mature_species_quant)] <- 0
...
```{r}
write.table(mirbase_mature_species_quant, file.path(projFolder, "QUANTIFICATION","mirbase_mature_species_star-counts.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)

write.table(data.frame(ID=rownames(mirbase_mature_species_quant.rpkm),mirbase_mature_species_quant.rpkm), file.path(projFolder, "QUANTIFICATION","mirbase_mature_species_star-rpkm.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```
...
for(i in 2:length(star.hairpin.files)){
    ...
    star.hairpin <- merge(star.hairpin, tmp.star.hairpin, by="miRNA", all.x=TRUE)
}
...
```{r}
barplot(apply(star.hairpin[,-1],2,sum), las=2, main="Reads aligned to Hairpin.fa", col=reportColours[1], cex.names=font.size)
```
...
for(i in 2:length(mirbase_hairpin_quant.files)){
  ...
  mirbase_hairpin_quant <- merge(mirbase_hairpin_quant, tmp, by="miRNA", all.x=TRUE)
}
mirbase_hairpin_quant[is.na(mirbase_hairpin_quant)] <- 0
...
```{r}
write.table(mirbase_hairpin_quant, file.path(projFolder, "QUANTIFICATION","mirbase_hairpin_star-counts.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
write.table(data.frame(ID=rownames(mirbase_hairpin_quant.rpkm),mirbase_hairpin_quant.rpkm), file.path(projFolder, "QUANTIFICATION","mirbase_hairpin_star-rpkm.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```
...
```{r}
barplot(apply(star.hairpin_species[,-1], 2, sum), las=2, main="Reads aligned to Hairpin_Species.fa", col=reportColours[1], cex.names=font.size)
```
```{r}
# Get the quantification
...
for(i in 2:length(mirbase_hairpin_species_quant.files)){
  ...
  mirbase_hairpin_species_quant <- merge(mirbase_hairpin_species_quant, tmp, by="miRNA", all=TRUE)
}
mirbase_hairpin_species_quant[is.na(mirbase_hairpin_species_quant)] <- 0
...
```{r}
write.table(mirbase_hairpin_species_quant, file.path(projFolder, "QUANTIFICATION","mirbase_hairpin_species_star-counts.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
write.table(data.frame(ID=rownames(mirbase_hairpin_species_quant.rpkm),mirbase_hairpin_species_quant.rpkm), file.path(projFolder, "QUANTIFICATION","mirbase_hairpin_species_star-rpkm.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```
...
```{r}
annotation <- importGTF(refAnnot.file, level=tOption)
```
...
```{r}
# Export the matrix to QUANTIFICATION folder
  write.table(fc.expr.star, file.path(projFolder, "QUANTIFICATION","reference_counts.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
  write.table(data.frame(ID=rownames(fc.expr.star.rpkm),fc.expr.star.rpkm), file.path(projFolder, "QUANTIFICATION","reference_rpkm.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
...
colnames(fc.expr.stats) <- gsub("_reference_star.bam", "", sapply(strsplit(colnames(fc.expr.stats), "Reference."),"[",2))
colnames(fc.expr.stats)[1] <- "Status"
...
if(length(biotypes)>1){
  ...
  counts_per_biotype <- apply(fc.expr.star[,-1], 2, sum)
}
...
```{r}
novelFile.info <- file.info(file.path(projFolder, "Reference", "novelLoci.bed"))

if(novelFile.info$size==0){
  novelDetected <- FALSE
} else {
  novelDetected <- TRUE
}
...
if(novelDetected){
  ...
  novelMirna <- data.frame(Chr=NA, Start=NA, Stop=NA)
}
```
...
if(novelDetected){
  ...
  for(i in 2:length(samples)){
    ...
    novel.quant <- merge(novel.quant, tmp[,c("Name", samples[i])], by="Name", all=TRUE)
  }
...
```{r}
write.table(novel.quant, file.path(projFolder, "QUANTIFICATION","novel_counts.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)

if(novelDetected){
  write.table(data.frame(ID=rownames(novel.rpkm),novel.rpkm), file.path(projFolder, "QUANTIFICATION","novel_rpkm.tsv"), sep="\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
}
```
...
```{r}
if(novelDetected) datatable(novel.quant)
```
### Annotation of the novel genes

These are the sequences of the predicted genes

```{r}
if(novelDetected) novel.fa <- importFA(file.path(projFolder, "Reference", "novelLoci.fa"))
```
...
for(i in 1:length(samples)){
  rawReads_perSample[i] <- sum(raw.reads[grep(paste0("^", samples[i]), names(raw.reads))])
}
names(rawReads_perSample) <- samples

qcRemovedReads_perSample <- c()
# THAT IS A QUICK FIX FOR SAMPLENAMES WITH TRUE SUBSETS!!! THIS IS NOT GENERIC, WHEN _S IS NOT PRESENT (OR PART OF THE FILENAME, THIS WILL FAIL) FIX THAT ASAP!!!!
samples_trimmed.reads <- sapply(strsplit(names(trimmed.reads),"_S"),"[",1)
for(i in 1:length(samples)){
  qcRemovedReads_perSample[i] <- rawReads_perSample[is.element(names(rawReads_perSample), samples[i])]-sum(trimmed.reads[is.element(samples_trimmed.reads, samples[i])])
}

refAssignedReads_perSample <- c() 
for(i in 1:length(samples)){
  refAssignedReads_perSample[i] <- fc.expr.stats[1,is.element(colnames(fc.expr.stats), samples.adj[i])]
}

refUnassigned_AmbiguityReads_perSample <- c() 
for(i in 1:length(samples)){
  refUnassigned_AmbiguityReads_perSample[i] <- fc.expr.stats[2,is.element(colnames(fc.expr.stats), samples.adj[i])]
}

refUnassigned_NoFeatureReads_perSample <- c() 
for(i in 1:length(samples)){
  refUnassigned_NoFeatureReads_perSample[i] <- fc.expr.stats[8,is.element(colnames(fc.expr.stats), samples.adj[i])]
}

refUnassigned_UnmappedReads_perSample <- c() 
for(i in 1:length(samples)){
  refUnassigned_UnmappedReads_perSample[i] <- fc.expr.stats[14,is.element(colnames(fc.expr.stats), samples.adj[i])]
}

mirbaseAssigned_species_MatureReads_perSample <- c()
adjustedColnames <- colnames(star.mature_species)
adjustedColnames <- sapply(strsplit(adjustedColnames, "_star"),"[",1)
for(i in 1:length(samples)){
  mirbaseAssigned_species_MatureReads_perSample[i] <- sum(star.mature_species[,is.element(adjustedColnames, samples.adj[i])])
}

mirbaseAssigned_species_HairpinReads_perSample <- c()
adjustedColnames <- colnames(star.hairpin_species)
adjustedColnames <- sapply(strsplit(adjustedColnames, "_star"),"[",1)
for(i in 1:length(samples)){
  mirbaseAssigned_species_HairpinReads_perSample[i] <- sum(star.hairpin_species[,is.element(adjustedColnames, samples.adj[i])])
}

contamination_trnaReads_perSample <- c()
adjustedColnames <- names(trna.mapped.reads)
adjustedColnames <- sapply(strsplit(adjustedColnames, "_tRNA"),"[",1)
for(i in 1:length(samples)){
  contamination_trnaReads_perSample[i] <- trna.mapped.reads[is.element(adjustedColnames, samples.adj[i])]
}

adjustedColnames <- names(phix.mapped.reads)
adjustedColnames <- sapply(strsplit(adjustedColnames, "_PhiX"),"[",1)
contamination_phixReads_perSample <- c()
for(i in 1:length(samples)){
  contamination_phixReads_perSample[i] <- phix.mapped.reads[is.element(adjustedColnames, samples.adj[i])]
}

reference_CountsPerBiotype_perSample <- counts_per_biotype[-c(1:2),]

if(is.null(nrow(reference_CountsPerBiotype_perSample))){
reference_CountsPerBiotype_perSample <-  t(as.matrix(reference_CountsPerBiotype_perSample))
}


# Need to adjust the colnames, in case there is a true subset available for the sample names
adjustedColnames <- colnames(counts_per_biotype)
adjustedColnames <- sapply(strsplit(adjustedColnames, "_star"),"[",1)
for(i in 1:length(samples)){
  reference_CountsPerBiotype_perSample[,i] <- counts_per_biotype[-c(1:2),is.element(adjustedColnames, samples[i])]
}
reference_CountsPerBiotype_perSample <- as.vector(reference_CountsPerBiotype_perSample)

# Now check still, how many of the NoFeatureReads were used to estiamte novel miRNA
novel_mirna_counts <- apply(novel.quant[,-c(1:4)], 2, sum)

overall_stats <- rbind(qcRemovedReads_perSample,
                       contamination_trnaReads_perSample,
                       contamination_phixReads_perSample,
                       mirbaseAssigned_species_MatureReads_perSample,
                       mirbaseAssigned_species_HairpinReads_perSample,
                       unlist(refUnassigned_AmbiguityReads_perSample),
                       refUnassigned_NoFeatureReads_perSample-novel_mirna_counts,
                       novel_mirna_counts,
                       reference_CountsPerBiotype_perSample,
                       refUnassigned_UnmappedReads_perSample
                       )

rownames(overall_stats)[7] <- "refUnassigned_NoFeatureReads_perSample (w/o novel)"
colnames(overall_stats) <- samples
#rownames(overall_stats) <- c("qcRemovedReads_perSample",
#                             "contamination_trnaReads_perSample",
#                             "contamination_phixReads_perSample",
#                             "mirbaseAssigned_MatureReads_perSample",
#                             "mirbaseAssigned_HairpinReads_perSample",
#                             "refUnassigned_AmbiguityReads_perSample",
#                             "refUnassigned_NoFeatureReads_perSample (w/o novel)",
#                             "novel_mirna_counts",
#                             "reference_CountsPerBiotype_perSample",
#                             "refUnassigned_UnmappedReads_perSample" )
...
threshold <- sort(apply(overall_stats,1,sum),decreasing = TRUE)[min(11, nrow(overall_stats))]
colorCodeSmall <- colorCode[apply(overall_stats,1,sum)>=threshold,]
legend("topright", inset=c(-0.4,0), legend=revVec(colorCodeSmall[,1]), fill=revVec(colorCodeSmall[,2]))
```
...
barplot(unconsideredReads, las=2, col=reportColours[1])
```
...
### Hierarchical clustering for tRNA (BOWTIE)

Here we use this object: star.mature_species.rpkm

```{r}
if(ncol(trna.quant.rpkm)>2){
  cluster.data <- trna.quant.rpkm
  #colnames(cluster.data) <- gsub("_star_mature_species.txt", "", colnames(cluster.data))
  trna.quant.rpkm.dist <- as.dist(1-cor(as.matrix(cluster.data)))
  hc <- hclust(trna.quant.rpkm.dist)           
  plot(hc)    
}
```


### Hierarchical clustering for Mature_species (STAR)

Here we use this object: star.mature_species.rpkm

```{r}
if(ncol(star.mature_species.rpkm)>2){
...
### Hierarchical clustering for protein coding genes (STAR)

Here we use this object: star.protein_coding.rpkm

```{r}

if(!is.null(star.protein_coding.rpkm)){
...
### Hierarchical clustering for Novel predictions (STAR)

Here we use this object: novel.rpkm


```{r}
if(novelDetected){
  if(ncol(novel.rpkm)>2){
...
```{r}
palette("default")
pairs(expr.PCA$x[,1:min(3, ncol(expr.PCA$x))])
```
...
if(sampleInfo.avail){
  if(ncol(sampleInfo)>1){
    for(i in 2:ncol(sampleInfo)){
      ...
      pairs(expr.PCA$x[,1:min(ncol(expr.PCA$x), 3)], col=pca.colours[,i], pch=20, oma=c(3,3,3,15))
      par(xpd=TRUE)
...
```{r}
palette("default")
pairs(expr.PCA$x[,1:min(ncol(expr.PCA$x), 3)])
```
...
```{r performPCA_trna}
expr.PCA <- prcomp(t(trna.quant.rpkm))
expr.PCA.novel <- expr.PCA
```
...
In case there is a column named 'DEgroup' inside the sampleInfo.tsv file, we will perform pairwise group comparisons here.

```{r import the data}
# Hairpin
  hp <- read.table(file.path(projFolder, "QUANTIFICATION", "mirbase_hairpin_species_star-counts.tsv"), header=TRUE)
  colnames(hp)[1] <- "gene"
  
# mature
  mature <- read.table(file.path(projFolder, "QUANTIFICATION", "mirbase_mature_species_star-counts.tsv"), header=TRUE)
  colnames(mature)[1] <- "gene"
  
# tRNA
  trna <- read.table(file.path(projFolder, "QUANTIFICATION", "trna_counts.tsv"), header=TRUE)
  colnames(trna)[1] <- "gene"
  
# reference
  ref <- read.table(file.path(projFolder, "QUANTIFICATION", "reference_counts.tsv"), header=TRUE)
  colnames(ref)[1] <- "gene"
  
# novel
  novel <- read.table(file.path(projFolder, "QUANTIFICATION", "novel_counts.tsv"), header=TRUE)
  colnames(novel)[1] <- "gene"
```
...
```{r remove low abundances}
# Remove low abundance rows
  geneCounts <- apply(allCounts[,-1], 1, sum)
  allCounts.filtered <- allCounts[geneCounts>abundanceRemove, ]
```

```{r calculate normalisation factors}
# calculate normalization factors
  allData <- DGEList(counts=allCounts[,-1])
  allData <- calcNormFactors(allData)
```

```{r get normcounts object}
# normalize counts using the normalization factors
  normCounts <- cpm(allData, normalized.lib.sizes=TRUE)
#  write.table(normCounts, file.path(projFolder, "RESULTS", "normalisedCounts.tsv"), sep="\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
```

```{r create group object}
if(do.DEtesting){
  group <- rep("UNKNOWN", ncol(allCounts)-1)
  realColnames <- strsplit(readLines(file.path(projFolder, "QUANTIFICATION", "mirbase_hairpin_species_star-counts.tsv"), n=1), "\t")[[1]][-1]

  foundGroups <- unique(sampleInfo$DEgroup)
  
  for(i in 1:length(foundGroups)){
    groupSamples <- sampleInfo$sampleID[sampleInfo$DEgroup==foundGroups[i]]
    
    group[which(is.element(realColnames, groupSamples))] <- paste0("Group",foundGroups[i])
  }  
...
```{r do de testing}
if(do.DEtesting){
  # perform differential expression analysis using negative binomial model
    design <- model.matrix(~group+0)
    y <- DGEList(counts=normCounts)
    y <- estimateDisp(y, design)
    fit <- glmQLFit(y, design)
}
```

```{r core function for DE testing}
  giveMeResults <- function(g1, g2, groupVector=gsub("group", "", colnames(fit$design)), fitIn=fit, FDR=de.fdr, filePath=NULL, biotype.split=TRUE){
    
    totalGroups <- length(groupVector)
    x <- rep(0, totalGroups)
    
    x[groupVector==g1] <- 1
    x[groupVector==g2] <- -1
    qlf <- glmQLFTest(fitIn, contrast = x)
    if(substr(qlf$comparison, 1,1)=="-"){
      comparison <- gsub(" ", "vs", gsub("-", "",gsub("1\\*group", "", qlf$comparison)))  
    } else {
      comparison <- gsub("group", "", gsub("^1\\*", "", gsub(" -1\\*", "vs", gsub("1*groupgroup", "", qlf$comparison))))
    }
    ...
    DEgenes <- topTags(qlf, n=nrow(normCounts))[["table"]]
    DEgenes <- data.frame(gene=rownames(DEgenes), DEgenes)
    
    out <- DEgenes[DEgenes$FDR<FDR,]
    
    if(!is.null(filePath)){
      write.table(out, file.path(filePath, paste0("edgeR-", comparison, ".tsv")), sep="\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
      
      if(biotype.split){
        out.hp <- out[is.element(out$gene, hp$gene),]
        write.table(out.hp, file.path(filePath, paste0("edgeR-", comparison, "-hp.tsv")), sep="\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
        
        out.mature <- out[is.element(out$gene, mature$gene),]
        write.table(out.mature, file.path(filePath, paste0("edgeR-", comparison, "-mature.tsv")), sep="\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
        
        out.trna <- out[is.element(out$gene, trna$gene),]
        write.table(out.trna, file.path(filePath, paste0("edgeR-", comparison, "-trna.tsv")), sep="\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
        
        out.ref <- out[is.element(out$gene, ref$gene),]
        write.table(out.ref, file.path(filePath, paste0("edgeR-", comparison, "-ref.tsv")), sep="\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
      
        out.novel <- out[is.element(out$gene, novel$gene),]
        write.table(out.novel, file.path(filePath, paste0("edgeR-", comparison, "-novel.tsv")), sep="\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
      }
...
```{r now do the DE testing}
if(do.DEtesting){
  for(g1 in 1:(length(foundGroups)-1)){
    for(g2 in (g1+1):length(foundGroups)){
      if(!dir.exists(file.path(projFolder, "RESULTS", "edgeR"))) dir.create(file.path(projFolder, "RESULTS", "edgeR"))
      out <- giveMeResults(g1=paste0("Group",foundGroups[g1]), g2=paste0("Group",foundGroups[g2]), filePath = file.path(projFolder, "RESULTS", "edgeR"))  
    }
  }
}
```
...
# Benchmarks
Do here the analysis of the snakemake benchmark output to optimise resource allocation
