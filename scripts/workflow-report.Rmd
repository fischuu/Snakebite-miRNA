...
if(!is.element("snakemake",ls())){
  ...
  pipelineConfig <- "/scratch/project_2001310/BovReg_TwinBullSperm/miRNA-pipeline_config-bovreg.yaml"
...
for(i in 1:length(samples)){
  qcRemovedReads_perSample[i] <- rawReads_perSample[i]-sum(trimmed.reads[grep(samples[i], names(raw.reads))])
}

refAssignedReads_perSample <- c() 
for(i in 1:length(samples)){
  refAssignedReads_perSample[i] <- fc.expr.stats[1,(i+1)]
}

refUnassigned_AmbiguityReads_perSample <- c() 
for(i in 1:length(samples)){
  refUnassigned_AmbiguityReads_perSample[i] <- fc.expr.stats[2,(i+1)]
}

refUnassigned_NoFeatureReads_perSample <- c() 
for(i in 1:length(samples)){
  refUnassigned_NoFeatureReads_perSample[i] <- fc.expr.stats[8,(i+1)]
}

refUnassigned_UnmappedReads_perSample <- c() 
for(i in 1:length(samples)){
  refUnassigned_UnmappedReads_perSample[i] <- fc.expr.stats[14,(i+1)]
}

mirbaseAssigned_MatureReads_perSample <- c()
for(i in 1:length(samples)){
  mirbaseAssigned_MatureReads_perSample[i] <- sum(star.mature_species[,(i+1)])
}

mirbaseAssigned_HairpinReads_perSample <- c()
for(i in 1:length(samples)){
  mirbaseAssigned_HairpinReads_perSample[i] <- sum(star.hairpin_species[,(i+1)])
}

contamination_trnaReads_perSample <- c()
for(i in 1:length(samples)){
  contamination_trnaReads_perSample[i] <- trna.mapped.reads[i]
}

contamination_phixReads_perSample <- c()
for(i in 1:length(samples)){
  contamination_phixReads_perSample[i] <- phix.mapped.reads[i]
}
...
```{r}
star.mature_species.rpkm.dist <- as.dist(1-cor(as.matrix(star.mature_species.rpkm)))
