...
```{r}
trimmed.read.files <- list.files(file.path(projFolder, "FASTQ", "TRIMMED"), pattern="*.wcl")
...
```{r}
trimmed.bases.files <- list.files(file.path(projFolder, "FASTQ", "TRIMMED"), pattern="*.wcc")
...
mature.unmapped.reads <- tmp$unmapped.reads
mature.mapped.reads <- tmp$mapped.reads
```
...
mature.unmapped.bases <- tmp$unmapped.reads
mature.mapped.bases <- tmp$mapped.reads
```
...
hairpin.unmapped.reads <- tmp$unmapped.reads
hairpin.mapped.reads <- tmp$mapped.reads
```
...
hairpin.unmapped.bases <- tmp$unmapped.reads
hairpin.mapped.bases <- tmp$mapped.reads
```

```{r}
par(mar=c(15,6,2,8), xpd=TRUE)
barplot(rbind(mature.unmapped.bases, hairpin.unmapped.bases, hairpin.mapped.bases), beside=TRUE, las=2, main="FMature.fa unaligned bases aligned to Hairpin.fa", col=c(4,5,6))
legend("topright", inset=c(-0.2,0), legend=c("Mature.fa (unmapped)","Hairpin.fa (unmapped)", "Hairpin.fa (mapped)"), fill=c(4,5,6))
```
...
reference.bowtie.unmapped.reads <- tmp$unmapped.reads
reference.bowtie.mapped.reads <- tmp$mapped.reads
```
...
reference.bowtie.unmapped.bases <- tmp$unmapped.reads
reference.bowtie.mapped.bases <- tmp$mapped.reads
```
